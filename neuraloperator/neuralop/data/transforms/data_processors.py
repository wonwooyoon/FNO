...
class DefaultDataProcessor(DataProcessor):
    ...
    """
    def __init__(
        self, in_normalizer=None, out_normalizer=None, meta_normalizer=None, meta_key: str = "meta"
    ):
        """
        Parameters
        ----------
        ...
        self.meta_normalizer = meta_normalizer
    ...
    def to(self, device):
        ...
        if self.meta_normalizer is not None:
            self.meta_normalizer = self.meta_normalizer.to(device)
        self.device = device
        return self
    ...
    def preprocess(self, data_dict, batched=True):
        ...
        if self.meta_key in data_dict and data_dict[self.meta_key] is not None:
            m = data_dict[self.meta_key].to(self.device)
            if self.meta_normalizer is not None:
                m = self.meta_normalizer.transform(m)
            data_dict[self.meta_key] = m
    ...
    def forward(self, **data_dict):
        ...
        if self.meta_key in data_dict:
            output = self.model(data_dict["x"], data_dict[self.meta_key])
        else:
