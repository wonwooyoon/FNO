...
# ==============================================================================
# Section 5: Normalization Distribution Check Functions
# (Called from FNO.py preprocess_data, not from generate_all_outputs)
# ==============================================================================
...
def check_normalization(
    ...
    """
    Perform complete normalization distribution check.

    Args:
        config: Configuration dictionary
        processor: Data processor with normalizers
        device: Device to use
        train_dataset: Training dataset
        val_dataset: Validation dataset
        test_dataset: Test dataset
    ...
    """
    print("\n" + "="*70)
    print("NORMALIZATION DISTRIBUTION CHECK")
    print("="*70)

    norm_config = config.get('OUTPUT', {}).get('NORM_CHECK', {})
    n_samples = norm_config.get('N_SAMPLES', 10)

    # Collect samples from test set
    if verbose:
        print(f"Collecting {n_samples} samples from test set...")
    ...
    for i in range(n_samples):
        ...
        samples_x.append(sample['x'])
        samples_y.append(sample['y'])

    # Stack samples
    x_batch = torch.stack(samples_x).to(device)  # (N, C, nx, ny, nt)
    y_batch = torch.stack(samples_y).to(device)  # (N, C, nx, ny, nt)
    ...
    x_before = x_batch.clone().cpu()
    y_before = y_batch.clone().cpu()

    x_after = processor.in_normalizer.transform(x_batch).cpu()
    y_after = processor.out_normalizer.transform(y_batch).cpu()

    # Analyze input distribution
    print("\n--- Analyzing Input Distribution ---")
    input_results = analyze_normalization_distribution(
        x_before, x_after, 'input', output_dir, config, verbose=verbose
    )

    # Analyze output distribution
    print("\n--- Analyzing Output Distribution ---")
    output_results = analyze_normalization_distribution(
        y_before, y_after, 'output', output_dir, config, verbose=verbose
    )
...
# ==============================================================================
# Section 6: Master Output Generation Function
# ==============================================================================
