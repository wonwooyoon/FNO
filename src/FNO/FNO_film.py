"""
FNO with FiLM (Feature-wise Linear Modulation) Training

This module implements training pipeline for TFNO models with FiLM (Feature-wise Linear 
Modulation) technique. Meta data is used for channel-wise modulation in the projection 
layer, allowing the model to adapt features based on conditional information such as 
permeability and porosity parameters.
"""
...
CONFIG = {
    ...
    'OUTPUT_DIR': './src/FNO/output_film',
    'N_EPOCHS': 10000,
    'EVAL_INTERVAL': 1,
    'TEST_SIZE': 0.1,
    'RANDOM_STATE': 42,
    ...
    'MODEL_CONFIG': {
        'in_channels': 8,  # Original input channels without meta data
        'out_channels': 1,
        'lifting_channel_ratio': 2,
        'projection_channel_ratio': 2,
        'positional_embedding': 'grid',
        'film_layer': True,  # Enable FiLM for channel-wise modulation
        'meta_dim': 2  # Meta data dimension for FiLM projection
    },
    'SCHEDULER_CONFIG': {
    ...
    'TRAINING_CONFIG': {
        ...
        'eval_model_path': './src/FNO/output_film/final/best_model_state_dict.pt'  # Path for eval mode
    },
    'OPTUNA_SEARCH_SPACE': {
...
def train_final_model(best_params: Dict[str, Any], train_dataset: CustomDataset, 
    ...
    output_path = Path(CONFIG['OUTPUT_DIR']) / 'FNO_film_compare.png'
    plot_compare(
        pred_phys, gt_phys, 
...
def run_optuna_training(train_dataset: CustomDataset, test_dataset: CustomDataset, 
...
def run_eval_mode(train_dataset: CustomDataset, test_dataset: CustomDataset, 
    ...
    with torch.no_grad():
        ...
        pred = model(in_normalizer.transform(x), meta_normalizer.transform(meta))
    ...
    output_path = Path(CONFIG['OUTPUT_DIR']) / 'FNO_film_eval_compare.png'
    plot_compare(
        pred_phys, gt_phys, 
        save_path=str(output_path), 
        sample_num=CONFIG['VISUALIZATION']['SAMPLE_NUM'], 
...
def run_training_mode(train_dataset: CustomDataset, test_dataset: CustomDataset, 
                     processor: DefaultDataProcessor, 
                     in_normalizer, out_normalizer, meta_normalizer, device: str) -> None:
...
def main() -> None:
    """Main training pipeline for FNO with FiLM modulation."""
    try:
        ...
        print(f"\n🚀 FNO-FiLM Training Pipeline Started")
