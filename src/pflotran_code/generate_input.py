...
sample = pd.read_csv("/home/geofluids/research/FNO/src/initial_others/output_lr/others.csv").values

sampled_pressure = sample[:, 0]
sampled_degra = sample[:, 1]

sampled_seawater = pd.read_csv("/home/geofluids/research/FNO/src/initial_seawater/output_lr/mixed_components.csv", header=None).values
...
for file_index in range(max_files):
    ...
    for i, line in enumerate(original_lines):
        ...
        if search_clinochlore in line:
            new_lines[i] = line.replace(search_clinochlore, f"FILENAME ../../initial_mineral/output_lr/clinochlore_cell_{file_index}.h5\n")
            continue
        if search_calcite in line:
            new_lines[i] = line.replace(search_calcite, f"FILENAME ../../initial_mineral/output_lr/calcite_cell_{file_index}.h5\n")
            continue
        if search_pyrite in line:
            new_lines[i] = line.replace(search_pyrite, f"FILENAME ../../initial_mineral/output_lr/pyrite_cell_{file_index}.h5\n")
            continue
        if search_perm in line:
            new_lines[i] = line.replace(search_perm, f"FILENAME ../../initial_perm/output_lr/perm_map_{file_index}.h5\n")
            continue
        if search_poro in line:
            new_lines[i] = line.replace(search_poro, f"FILENAME ../../initial_perm/output_lr/poro_map_{file_index}.h5\n")
            continue
    ...
    output_file_path = f"/home/geofluids/research/FNO/src/pflotran_code/output_lr/pflotran_{file_index}.in"
