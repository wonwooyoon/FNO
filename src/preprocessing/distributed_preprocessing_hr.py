...
Only executes preprocessing_revised_hr.py on remote servers
"""
...
def sync_preprocessing_script(host: str, user: str, port: int):
    """Sync preprocessing_revised_hr.py from git repository on remote server"""

    # Git command to fetch and checkout only the preprocessing_revised.py file
    sync_command = (
        "cd research/FNO && "
        "git fetch origin && "
        "git checkout origin/master -- src/preprocessing/preprocessing_revised_hr.py"
    )
    ...
    print(f"Syncing preprocessing_revised_hr.py on {host}:{port} from git...")
...
def execute_remote_preprocessing(host: str, user: str, port: int, output_suffix: str):
    """Execute preprocessing_revised_hr.py on remote server via SSH"""

    # SSH command to run preprocessing_revised.py with simplified input
    # Use single line command with && separators
    remote_command = f"cd research/FNO && source .venv_FNO/bin/activate && python3 src/preprocessing/preprocessing_revised_hr.py {output_suffix}"
...
def download_result_file(host: str, user: str, port: int, output_suffix: str):
    ...
    remote_file = f"research/FNO/src/preprocessing/input_output_hr_com{output_suffix}.pt"
    local_file = f"./src/preprocessing/input_output_hr_com{output_suffix}.pt"
...
def sync_local_preprocessing_script():
    """Sync preprocessing_revised_hr.py from git repository locally"""
    ...
    print(f"Syncing preprocessing_revised_hr.py locally from git...")
...
def execute_local_preprocessing(output_suffix: str):
    """Execute preprocessing_revised_hr.py locally using the same environment"""

    # Local command to run preprocessing_revised.py
    # Use same paths and virtual environment as remote servers
    local_command = f"python3 src/preprocessing/preprocessing_revised_hr.py {output_suffix}"
...
def main():
    ...
    # Show local file
    local_file = f"./src/preprocessing/input_output_hr_comlocalhost.pt"
    ...
    for server in config['servers']:
        host = server['host']
        output_suffix = f"{host}"
        remote_file = f"./src/preprocessing/input_output_hr_com{output_suffix}.pt"
