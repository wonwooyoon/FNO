#!/usr/bin/env python3
"""
Test FNO_pure.py optuna mode with fixed tuple handling
"""
...
def test_fno_optuna_fixed():
    print("="*60)
    print("Testing FNO_pure.py - OPTUNA mode (Fixed tuple handling)")
    print("="*60)
    ...
    try:
        ...
        FNO_pure.CONFIG.update({
            ...
            'OUTPUT_DIR': './test_output_fno_optuna_fixed',
            ...
            'TRAINING_CONFIG': {
                ...
                'eval_model_path': './test_output_fno_optuna_fixed/final/best_model_state_dict.pt'
            },
            'VISUALIZATION': {
                'SAMPLE_NUM': 8,
                'TIME_INDICES': (4, 9, 14, 19),
                'DPI': 200,
                'SAVEASCSV': False  # Disable CSV for faster test
            }
        })
        ...
        # Run the test
        print("\nRunning FNO_pure.py optuna mode test (fixed)...")
        FNO_pure.main()
        
        print("\n✅ FNO_pure.py optuna mode test (fixed) PASSED!")
        
    except Exception as e:
        print(f"\n❌ FNO_pure.py optuna mode test (fixed) FAILED: {e}")
        import traceback
        traceback.print_exc()
        raise
